-- =====================================================
-- MIGRAZIONE 012: Aggiornamento struttura tabella products
-- Allinea i nomi dei campi con le intestazioni del file Excel
-- "20251015 Listino Nomenclatura.xlsx"
-- =====================================================

-- Backup della tabella esistente per sicurezza
CREATE TABLE IF NOT EXISTS public.products_backup AS 
SELECT * FROM public.products;

-- Rimuovi i vincoli esistenti prima di rinominare le colonne
...
-- Il campo is_active esiste già, non serve rinominarlo
...
-- Crea un nuovo indice unico per il campo apcpro (ex code)
CREATE UNIQUE INDEX IF NOT EXISTS products_apcpro_unique ON public.products(apcpro);

-- Aggiorna i vincoli per il nuovo nome del campo
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS products_code_check;
ALTER TABLE public.products ADD CONSTRAINT products_apcpro_check CHECK (LENGTH(apcpro) > 0);

-- Aggiorna i vincoli per il nuovo nome del campo descrizione
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS products_name_check;
ALTER TABLE public.products ADD CONSTRAINT products_descrizione_check CHECK (LENGTH(descrizione) > 0);

-- Aggiorna i vincoli per il prezzo
ALTER TABLE public.products DROP CONSTRAINT IF EXISTS products_base_price_check;
ALTER TABLE public.products ADD CONSTRAINT products_apprli_check CHECK (apprli > 0);
